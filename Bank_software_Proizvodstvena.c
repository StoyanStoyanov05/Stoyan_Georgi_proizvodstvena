...
int main()
{
	...
	FILE *f;
	int key,br=0,i;char filename[40],numb;
...
void withdraw(DATA *D,int num)
 {
    ...
    if(D[num].balance>=amount){
        ...
        printf("Uspeshno teglene. Balance: %d\n", D[num].balance);
        return;
    }else{
...
void deposit(DATA *D, int num)
 {
    ...
    scanf("%d",&amount);
    D[num].balance=D[num].balance + amount;
    printf("Uspeshen deposit. Balance: %d\n", D[num].balance);
    return;
...
void transfer(DATA *D, int num)
 {
    ...
    printf("Uspeshen transfer. Balance: %d\n", D[num].balance);
    return;
...
void second_menu(DATA *D, int num)
{
	...
	do
	{
		...
		printf("2 - Deposit\n");
		printf("3 - Transfer\n");
		printf("4 - Obrabotvane na tranzakcia\n");
		printf("5 - Logout\n");
		printf("Kod na operacia:");
		scanf("%d",&key);
		switch(key){
